...
#define LIBWPROFINJ_PYTRACE_EVENTS_PATH_FMT "wprofinj-pytrace.%d.%d.events"
...
static struct injectee *injmgr_add_injectee(struct tracee_state *tracee)
{
	...
	inj->pytrace_respool_fd = -1;
	inj->pytorch_events_fd = -1;
	inj->pytorch_respool_fd = -1;
...
static int injmgr_send_pytrace_setup(struct injectee *inj, int workdir_fd)
{
	...
	snprintf(events_path, sizeof(events_path), LIBWPROFINJ_PYTRACE_EVENTS_PATH_FMT, getpid(), inj->pid);
	snprintf(respool_path, sizeof(respool_path), LIBWPROFINJ_PYTRACE_RESPOOL_PATH_FMT, getpid(), inj->pid);

	events_fd = openat(workdir_fd, events_path, O_RDWR | O_CREAT | O_CLOEXEC, 0644);
	if (events_fd < 0) {
		err = -errno;
		eprintf("Failed to create PyTrace event dump for %s at '%s': %d\n", injectee_str(inj), events_path, err);
		return err;
	}
	respool_fd = openat(workdir_fd, respool_path, O_RDWR | O_CREAT | O_CLOEXEC, 0644);
	if (respool_fd < 0) {
		err = -errno;
		eprintf("Failed to create PyTrace resource pool for %s at '%s': %d\n", injectee_str(inj), respool_path, err);
		close(events_fd);
		return err;
	}
	...
	inj->pytrace_events_fd = events_fd;
	inj->pytrace_events_path = strdup(events_path);
	inj->pytrace_respool_fd = respool_fd;
	inj->pytrace_respool_path = strdup(respool_path);
	return 0;
}
...
static int injmgr_send_pytorch_setup(struct injectee *inj, int workdir_fd)
{
	...
	snprintf(events_path, sizeof(events_path), LIBWPROFINJ_PYTORCH_EVENTS_PATH_FMT, getpid(), inj->pid);
	snprintf(respool_path, sizeof(respool_path), LIBWPROFINJ_PYTORCH_RESPOOL_PATH_FMT, getpid(), inj->pid);

	events_fd = openat(workdir_fd, events_path, O_RDWR | O_CREAT | O_CLOEXEC, 0644);
	if (events_fd < 0) {
		err = -errno;
		eprintf("Failed to create PyTorch event dump for %s at '%s': %d\n", injectee_str(inj), events_path, err);
		return err;
	}
	respool_fd = openat(workdir_fd, respool_path, O_RDWR | O_CREAT | O_CLOEXEC, 0644);
	if (respool_fd < 0) {
		err = -errno;
		eprintf("Failed to create PyTorch resource pool for %s at '%s': %d\n", injectee_str(inj), respool_path, err);
		close(events_fd);
		return err;
	}
	...
	inj->pytorch_events_fd = events_fd;
	inj->pytorch_events_path = strdup(events_path);
	inj->pytorch_respool_fd = respool_fd;
	inj->pytorch_respool_path = strdup(respool_path);
	return 0;
}
