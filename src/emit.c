...
static u64 *pmu_cpu_val_ts;		/* [pmu_event_cnt][cpu_cnt] when that previous sample was taken */
...
int init_emit(struct worker_state *w)
{
	...
	if (env.pmu_event_cnt > 0 && env.pmu_real_cnt > 0) {
		...
		pmu_cpu_val_ts = calloc(cells, sizeof(*pmu_cpu_val_ts));
	}
...
static void json_pmu_counters(struct json_state *j, const struct pmu_val *st_ctrs,
			      const struct pmu_val *ev_ctrs, bool diffs, u64 dt_ns)
{
	if (!ev_ctrs)
		return;
	if (!diffs && !st_ctrs)
...
static void emit_pmu_event(struct worker_state *w, const struct wevent *e,
			   const struct pmu_val *st_ctrs, u64 dt_ns)
{
	struct wprof_task task = wevent_resolve_task(w->dump_hdr, e->task_id);
	...
	emit_instant(track_uuid, e->ts, iid_str(pmu->name_iid, pmu->name), IID_CAT_PMU_EVENT) {
		...
		emit_perf_counters(st_ctrs, pmu_vals, false, dt_ns);
		emit_callstack(w, tr_id);
	}
}
...
static int process_pmu_event(struct worker_state *w, const struct wevent *e)
{
	...
	if (env.pmu_real_cnt) {
		...
		prev_ts = &pmu_cpu_val_ts[cell];
		st_ctrs = *seen ? prev : NULL;
		dt_ns = *seen ? e->ts - *prev_ts : 0;
	}

	if (env.json_path)
		emit_pmu_event_json(w, e, st_ctrs, dt_ns);
	else
		emit_pmu_event(w, e, st_ctrs, dt_ns);
...
static void emit_switch(struct worker_state *w, const struct wevent *e, struct switch_ctx *s)
{
	...
	if (s->trace_next && !is_idle_task(&task) && s->prev_state == WTRS_PREEMPTED) {
		...
		emit_instant(trackid_thread(&next), e->ts, IID_NAME_PREEMPTOR, IID_CAT_PREEMPTOR) {
			...
			if (env.emit_tidpid) {
				emit_kv_int(IID_ANNK_WAKEE_TID, task_tid(&task));
				emit_kv_int(IID_ANNK_WAKEE_PID, task.pid);
			}
	...
	emit_slice_end(trackid_thread(&task),
		...
		emit_perf_counters(s->prev_oncpu_ctrs, s->pmu_vals, false /* !diffs */,
				   s->prev_oncpu_ts ? e->ts - s->prev_oncpu_ts : 0);
...
static void emit_waking(struct worker_state *w, const struct wevent *e, enum waking_flags flags)
{
	...
	/*
	 * Waker instant on the waker's timeline at wake time; its flow id (the wake
	 * timestamp) pairs it with the wakee side rendered at that thread's
	 * switch-in. Preemption has no EV_WAKING and is rendered at the switch
	 * instead (see emit_switch).
	 */
...
static void emit_softirq_exit(struct worker_state *w, const struct wevent *e)
{
	...
	emit_slice_end(trackid_thread(&task),
		...
		emit_perf_counters(NULL, pmu_vals, true /* diffs */, e->ts - e->softirq.softirq_ts);
	}
}
...
static void emit_wq_end(struct worker_state *w, const struct wevent *e)
{
	...
	emit_slice_end(trackid_thread(&task),
		...
		emit_perf_counters(NULL, pmu_vals, true /* diffs */, e->ts - e->wq.wq_ts);
	}
}
...
static u64 ensure_req_track(const struct wprof_task *t, u64 req_id, const char *req_name)
{
	...
	/*
	 * req task events know req_id but not request name, but they might arrive before
	 * REQ_BEGIN/REQ_SET events, so we defer track descriptor emit until we get request name
	 * known
	 */
	if (!s->exists && req_name) {
			   ...
			   const struct pmu_val *req_ctrs, u64 req_oncpu_ns, u64 req_offcpu_ns)
{
	...
	case REQ_SET:
		...
		if (env.emit_req_embed) {
			emit_slice_begin(thread_req_track_uuid, e->ts, iid_str(thread_req_name_iid, thread_req_name), IID_CAT_REQUEST_THREAD) {
				emit_kv_str(IID_ANNK_REQ_NAME, iid_str(req_name_iid, req_name));
				emit_kv_int(IID_ANNK_REQ_ID, e->req.req_id);
				if (first_embed_event)
					emit_flow_id(req_id);
...
{
	struct track_state *s = track_state_get_or_add(DTK_CUDA_PROC_STREAM, pid, stream_id);
...
static void emit_cuda_api(struct worker_state *w, const struct wevent *e)
{
	...
	bool py_split = env.capture_pytorch && !(env.emit_py_combine && env.capture_pytrace);
	enum dyn_track_kind py_kind = py_split ? DTK_PYTORCH : DTK_PYTRACE;
...
static int process_pytorch(struct worker_state *w, const struct wevent *e)
{
	 ...
	 * slice nesting and truncating the long ProfilerStep slice. Drop them from
	 * the emitted trace (combined and split paths alike); they remain in the
	 * JSON output path.
	 */
	...
	if (env.emit_py_combine && env.capture_pytrace)
...
/*
 * Resolve the cfg that owns an event's args: for a generic ~~ span it's the
 * firing leg (entry/exit); *ret_filter says whether ret/non-ret filtering
 * applies (only native spans split their args across the two events).
 */
static const struct utrace_cfg *utrace_arg_cfg(const struct wevent *e, bool *ret_filter)
{
	const struct utrace_cfg *cfg = &env.utrace_cfgs[e->utrace.utrace_id];

	*ret_filter = cfg_is_span(cfg) && cfg->type != UTRACE_SPAN;
	if (cfg->type == UTRACE_SPAN)
		cfg = (e->kind == EV_UTRACE_EXIT) ? cfg->span.exit : cfg->span.entry;
	return cfg;
}

static bool utrace_arg_for_event(const struct wevent *e, bool ret_filter, const struct utrace_param *p)
{
...
static void emit_utrace_event(struct worker_state *w, const struct wevent *e)
{
	...
	int arg_cnt = 0;
	for (int i = 0; i < arg_cfg->param_cnt; i++) {
...
static void emit_utrace_json(struct worker_state *w, const struct wevent *e)
{
	...
	if (arg_cnt > 0) {
		...
		int arg_idx = 0;
		for (int i = 0; i < arg_cfg->param_cnt && arg_idx < arg_cnt; i++) {
			const struct utrace_param *p = &arg_cfg->params[i];
			if (!utrace_arg_for_event(e, ret_filter, p))
				continue;
