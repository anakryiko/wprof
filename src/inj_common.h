...
struct inj_run_ctx {
	...
	long sess_start_ts;
	long sess_end_ts;
	u64 fr_chunk_size;	/* flight recorder: rotate event chunks every N bytes (0 = off) */
...
enum inj_msg_kind {
	...
	INJ_MSG_SHUTDOWN = 6,
	INJ_MSG_CHUNK_DONE = 7,		/* injectee -> wprof: a rotated event chunk is complete */
	INJ_MSG_CHUNK_FD = 8,		/* wprof -> injectee: a fresh event chunk fd (spare) */
};
...
struct inj_msg {
	...
	union {
		...
		struct inj_msg_chunk_done {
			enum inj_feature feature;	/* which feature's event chunk completed */
